...
Each checksum is a SHA-256 of the SQL generated by that migration's `up()` method (line endings normalized to `\n`), similar to Flyway/Liquibase hashing SQL files rather than application source.

If a stored checksum no longer matches, a `MigrationChecksumMismatchError` is thrown before further migrations run. The error includes the SQL used to compute the current checksum so you can tell a real change from environment/driver differences.
