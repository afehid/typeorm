...
export class MigrationExecutor {
    ...
    /**
     * Collects SQL generated by `up()` via QueryRunner SQL memory.
     * When `execute` is true, statements still run against the database.
     * When false, SQL-memory mode records schema SQL without applying it.
     *
     * @param queryRunner
     * @param run
     * @param execute
    ...
    protected async captureMigrationSql(
        ...
        if (!execute) {
            ...
            try {
                await run()
                return this.sqlMemoryToChecksumInput(queryRunner)
            } finally {
    ...
    protected sqlMemoryToChecksumInput(queryRunner: QueryRunner): string[] {
        return queryRunner
            .getMemorySql()
            .upQueries.map((item) =>
                formatSqlForChecksum(item.query, item.parameters),
            )
    }
